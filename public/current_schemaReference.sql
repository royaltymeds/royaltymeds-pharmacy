...
CREATE TABLE public.restock_requests (
  ...
  status text NOT NULL DEFAULT 'pending'::text CHECK (status = ANY (ARRAY['requested'::text, 'linked_to_po'::text, 'received'::text, 'cancelled'::text, 'on_hold'::text])),
  total_amount numeric,
...
CREATE TABLE public.restock_items (
  ...
  held_from_request_id uuid,
  hold_status text NOT NULL DEFAULT 'active'::text CHECK (hold_status = ANY (ARRAY['active'::text, 'on_hold'::text])),
  ...
  CONSTRAINT restock_items_held_from_request_id_fkey FOREIGN KEY (held_from_request_id) REFERENCES public.restock_requests(id),
  CONSTRAINT restock_items_held_by_fkey FOREIGN KEY (held_by) REFERENCES public.users(id)
);
